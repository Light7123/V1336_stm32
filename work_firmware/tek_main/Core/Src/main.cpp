...
void modem_status()
{
	...
	HAL_UARTEx_ReceiveToIdle_DMA(&huart1,  (uint8_t*)buffer, 500);
	...
	for(int r=r_last;r<490;r++)
	{
		...
		//		if(buffer[r]=='L'&&buffer[r+1]=='O'&&buffer[r+2]=='G'&&buffer[r+3]==':')
		//		{
		//			for(int iii=r;iii<480;iii++)
		//			{
		//				if(buffer[r]=='\r')
		//				{
		//					HAL_UART_Transmit(&huart3, (uint8_t*)"FROM_SIM: ", 9,50);
		//					HAL_UART_Transmit(&huart3, (uint8_t*)log_buf.c_str(), 500,300);
		//					HAL_UART_Transmit(&huart3, (uint8_t*)"\n\r", 2,50);
		//					r_last=r;
		//					break;
		//				}
