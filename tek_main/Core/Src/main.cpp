...
/* Definitions for ModemUart */
osThreadId_t ModemUartHandle;
const osThreadAttr_t ModemUart_attributes = {
		.name = "ModemUart",
		.stack_size = 128 * 4,
		.priority = (osPriority_t) osPriorityNormal,
};
...
osThreadId_t WorkerHandle;
const osThreadAttr_t Worker_attributes = {
		.name = "Worker",
		.stack_size = 128 * 4,
		.priority = (osPriority_t) osPriorityNormal,
};
/* Definitions for modbas_to_mqtt */
osMessageQueueId_t modbas_to_mqttHandle;
const osMessageQueueAttr_t modbas_to_mqtt_attributes = {
		.name = "modbas_to_mqtt"
};
/* USER CODE BEGIN PV */
...
/* USER CODE BEGIN Header_ModemUartFunc */
/**
 * @brief Function implementing the ModemUart thread.
 * @param argument: Not used
 * @retval None
 */
...
void ModemUartFunc(void *argument)
{
	...
	for(;;)
	{
		...
		if(uxQueueMessagesWaiting(modbus_to_mqtt_queue)>1)
		{
			for(int i=0; i<dev.size();i++)
			{
				if(!(dev[i].modbus_result==dev[i].old_modbus_res))
				{
					dev[i].old_modbus_res=dev[i].modbus_result;
					mqtt_send(dev[polled_reg].topic,dev[polled_reg].modbus_result);
		...
		if(!modemRx.empty())
		{
			xQueueSend(mqtt_to_modbus_queue,&toSend,100);
		}
		osDelay(100);
	}
	/* USER CODE END ModemUartFunc */
}

/* USER CODE BEGIN Header_modbus_write */
/**
...
void modbus_write(void *argument)
{
	...
	for(;;)
	{
		for(int i=0;i<dev.size();i++)//TODO получение количества опрашиваемых регистров
		{
			...
			if(time_op.Minutes-dev[i].last_time.Minutes>=dev[i].pull_interval)
			{
				...
				if(time_op.Minutes-dev[i].last_time.Minutes<=0)
				{
	...
	/* USER CODE END modbus_write */
}

/* USER CODE BEGIN Header_Worker_func */
/**
 * @brief Function implementing the Worker thread.
 * @param argument: Not used
 * @retval None
 */
